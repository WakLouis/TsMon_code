...
# 示例数据（请替换为您自己的数据）
x = np.arange(0, 4)
y1 = [0.987, 0.986, 0.987, 0.986]
y2 = [0.934, 0.967, 0.967, 0.982]
...
line1, = ax.plot(x, y1, '-', label='TsMon', marker='o')
line2, = ax.plot(x, y2, '--', label='Count-Min Sketch', marker='h')
...
# 设置轴刻度
ax.xaxis.set_major_locator(MultipleLocator(1))  # 主刻度为2
...
# 保存图表（按需调整文件格式和分辨率）
plt.savefig('./Reconstruction.pdf', bbox_inches='tight')  # PDF适合论文提交
