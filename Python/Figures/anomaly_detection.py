...
# 自定义颜色循环（更适合区分）
colors = ['#7ec2ff', '#feca31', '#2ca02c', '#d62728',
...
y1 = [0.908, 0.966, 0.986, 0.624, 0.937, 0.999, 0.988, 0.995]
y2 = [0.951, 0.448, 0.515, 0.068, 0.31, 0.976, 0.351, 0.832]
# y2 = [381000]
...
# 绘制折线
# plt.yscale('log')
bars = plt.bar(x - 0.125, y1, width=0.25, hatch='//',
               edgecolor='white', label='Accuracy')
bars = plt.bar(x + 0.125, y2, width=0.25, hatch='//',
               edgecolor='white', label='F1 Score')
plt.xticks(x, ['TsMon', 'HSTD', 'Diff-1', 'Chi-Squared Test', 'IQR', 'Z-Score', 'KNN', 'AE'])
...
ax.set_ylabel('Accuracy and F1-Score', fontweight='bold')
...
# 保存图表（按需调整文件格式和分辨率）
plt.savefig('./anomaly_detection.pdf', bbox_inches='tight')  # PDF适合论文提交
